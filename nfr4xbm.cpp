...
// Open Remote Control
		...
		{
			printf("NFR4Xboot <open remote control>");
	...
	f = fopen("/var/lib/opkg/status", "rt");
   	if (f) {
		while (fgets(buf, 256, f)) {
			...
			if (strstr(buf, "Package: ini-dvb-modules-ini-900")) {
				fgets(buf, 256, f);
				temps = string(buf);
				pos = temps.find(' ');
				if(pos != string::npos)
			...
			if (strstr(buf, "Package: ini-dvb-modules-ini-800")) {
				fgets(buf, 256, f);
				temps = string(buf);
				pos = temps.find(' ');
				if(pos != string::npos)
			...
			if (strstr(buf, "Package: ini-dvb-modules-inihde")) {
				fgets(buf, 256, f);
				temps = string(buf);
				pos = temps.find(' ');
				if(pos != string::npos)
			...
			if (strstr(buf, "Package: odin-dvb-modules-odinm7s")) {
				fgets(buf, 256, f);
				temps = string(buf);
				pos = temps.find(' ');
				if(pos != string::npos)
			...
			if (strstr(buf, "Package: ceryon-dvb-modules-7100s")) {
				fgets(buf, 256, f);
				temps = string(buf);
				pos = temps.find(' ');
				if(pos != string::npos)
...
// Load background Pixmap
	std::string pic = "/usr/lib/enigma2/python/Plugins/Extensions/NFR4XBoot/icons/nfr4xmenu.png";
...
void nfr4xbm::drawmenu()
{
	...
	display->RenderString(nfr4xbootver, 70, 365, 500, fbClass::LEFT, 22, 255, 255, 255);
	...
	display->RenderString("NFR4XBoot is running on:", 70, 450, 300, fbClass::LEFT, 20, 255, 255, 255);
	if(model == "dm7025")
...
void nfr4xbm::startTimer()
{
	current_time = time(0);
}

int nfr4xbm::checkTimer()
{
