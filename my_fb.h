...
public:
	...
	void blit();
	void myend();
	int SetSAA(int value);
	int SetMode(unsigned int nxRes, unsigned int nyRes, unsigned int nbpp);
	int SetMode7025(unsigned int nxRes, unsigned int nyRes, unsigned int nbpp);
#ifdef USEFREETYPEFB
	...
	void W_buffer(int sx, int sy, int width, int height);
	void Fill_buffer(int sx, int sy, int width, int height);
};
